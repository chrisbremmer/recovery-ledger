...
- **D-01:** Eight tables in v1: `oauth_tokens`, `sync_runs`, `cycles`, `recoveries`, `sleeps`, `workouts`, `daily_summaries`, `decisions`. Per-entity hybrid shape — normalized columns for hot-path fields (`score_state`, `start`, `end`, `timezone_offset`, `updated_at`, plus the SCORED-only numeric scores) **plus** a `raw_json TEXT NOT NULL` column for every WHOOP-sourced row. Profile + body_measurement are low-volume → stored as a single-row `profile` table and a `body_measurements` history table (one row per WHOOP-returned measurement) with mostly-raw_json shape. `decisions` is irreplaceable user data — separate backup posture per Pitfall 7.
...
- **D-12:** **Research item — confirm whether WHOOP v2 endpoints accept an `updated_since` (or equivalent) filter parameter.** If they do, the HTTP query uses it directly. If they do not, the resource client paginates from `start >= since` and post-filters in the resource module after parse. Researcher confirms against `developer.whoop.com/api` for each of cycles, recovery, sleep, workouts.
...
- **Cursor query is `MAX(updated_at) FROM <resource_table>`, NOT `MAX(updated_at) FROM <resource_table> WHERE updated_at IS NOT NULL`.** If `updated_at` is nullable for any reason (e.g., legacy data, fixture rows), the COALESCE-to-zero behavior is what we want — the cursor falls back to "fetch everything." Safer than potentially skipping a backfill.
