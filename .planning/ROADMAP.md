...
**Success Criteria** (what must be TRUE):
  ...
  5. `bootstrap.ts` shrinks by ≥ 80 lines (target ≤ 250 deferred to Phase 12 — see 10-RESEARCH.md Q2-RESOLVED); `productionWhoopFetcher`, `whoopErrorKindToStatus`, and `services_runDoctor` live in `src/services/doctor/wiring.ts`; doctor checks use required-deps DI matching non-doctor services; `src/services/api-gap.ts` is a single file (no directory) and `API_GAP_ENTRIES` lives at `src/domain/api-gap/catalog.ts`.
**Plans**: 5 plans (ARCH-02 + ARCH-03 collapsed into single PR to avoid broken-main runtime window between PR2 and PR3 — see 10-RESEARCH.md Q5-RESOLVED)
- [ ] 10-01-arch-01-sanitize-to-domain-PLAN.md — Move `sanitize` from `infrastructure/observability/` to `domain/observability/`; rewrite 23 importers; add Gate H (ARCH-01)
- [ ] 10-02-arch-02-03-singletons-and-client-di-PLAN.md — Drop tokenStore + refreshOrchestrator singletons; bootstrap owns construction; invert `client.ts` via `authedCall` DI parameter; 6 resource modules become factories; ADR-0002 §Enforcement amendment; auth.ts uses direct `createTokenStore()` (documented exception); add Gates I + J; verify ARCH-04 + ARCH-05 closed-state holds (ARCH-02, ARCH-03, ARCH-04, ARCH-05)
- [ ] 10-03-arch-06-doctor-wiring-extract-PLAN.md — Extract `productionWhoopFetcher` + `whoopErrorKindToStatus` + `services_runDoctor` from bootstrap.ts to `src/services/doctor/wiring.ts`; add unit test; add Gate K (ARCH-06)
- [ ] 10-04-arch-07-doctor-required-deps-PLAN.md — Audit all 14 doctor checks; tighten optional `deps?` → required `deps`; drop `?? tokenStore.read()` fallbacks; wire explicit AuthProbeDeps + TokenFreshnessProbeDeps in `wiring.ts` (ARCH-07)
- [ ] 10-05-arch-08-api-gap-inline-PLAN.md — Collapse `src/services/api-gap/` directory to single `src/services/api-gap.ts`; promote `API_GAP_ENTRIES` catalog to `src/domain/api-gap/catalog.ts`; rewrite 7 importers; add Gate L (ARCH-08)
**PR boundaries**: 5 PRs in build order (ARCH-01 → ARCH-02+03 (combined) → ARCH-06 → ARCH-07 → ARCH-08; ARCH-04 + ARCH-05 fold into the combined ARCH-02+03 PR)
**UI hint**: no
