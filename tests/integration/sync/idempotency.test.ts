...
//
// Phase 10 ARCH-03: the test composes each resource module factory with a
// deterministic fake `authedCall` instead of mocking the (now-deleted)
// `services/refresh-orchestrator` singleton. The MSW interceptor still
// covers every fetch; the fake authedCall just invokes the operation with
// a fixed test token.
