...
describe('MCP stdout purity (dist smoke)', () => {
  test('dist/mcp.mjs stdout contains only valid JSON-RPC, with sanitized tool responses', async () => {
    ...
    const watchedSources = [
      ...
      // Phase 4 added 7 new tool files. A stale dist that does not yet
      // reflect changes to any of these would silently mask the change
      // under test. Watch the whole set so the stale-dist warning fires
      // for any tool-level edit.
