// MSW helper for the WHOOP `/v2/recovery` endpoint. Sibling to
// `msw-whoop-cycles.ts`; same shape, different endpoint + fixtures dir.
// See `msw-whoop-cycles.ts` for the full rationale (D-21 GET-only,
// fixture-per-request loading, one-shot override seam, default rate-limit
// headers).
...
export const RECOVERY_URL = 'https://api.prod.whoop.com/v2/recovery';
...
export interface WhoopRecoveryHelper {
  server: SetupServer;
  getHitCount(): number;
  resetHitCount(): void;
...
export function createWhoopRecoveryHelper(): WhoopRecoveryHelper {
  let hitCount = 0;
  let nextResponse: NextResponse | null = null;
  let lastRequestUrl: URL | null = null;

  const handler: HttpHandler = http.get(RECOVERY_URL, ({ request }) => {
    hitCount += 1;
