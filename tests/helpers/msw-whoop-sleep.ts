// MSW helper for the WHOOP `/v2/activity/sleep` endpoint. Sibling to
...
export const SLEEP_URL = 'https://api.prod.whoop.com/v2/activity/sleep';
...
export interface WhoopSleepHelper {
  server: SetupServer;
  getHitCount(): number;
  resetHitCount(): void;
...
export function createWhoopSleepHelper(): WhoopSleepHelper {
  let hitCount = 0;
  let nextResponse: NextResponse | null = null;
  let lastRequestUrl: URL | null = null;

  const handler: HttpHandler = http.get(SLEEP_URL, ({ request }) => {
    hitCount += 1;
