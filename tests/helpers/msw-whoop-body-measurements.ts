// MSW helper for the WHOOP `/v2/user/measurement/body` endpoint. Sibling
// to `msw-whoop-cycles.ts`; same shape, different endpoint + fixtures
// dir. Like profile, body-measurements is single-shot history (one record
// per measurement event; A4) — the schema does not wrap in
// `{records, next_token}`. See `msw-whoop-cycles.ts` for the full
// rationale.
...
export const BODY_MEASUREMENTS_URL = 'https://api.prod.whoop.com/v2/user/measurement/body';

const FIXTURES_DIR = join(process.cwd(), 'tests', 'fixtures', 'whoop', 'body-measurements');
...
export interface WhoopBodyMeasurementsHelper {
  server: SetupServer;
  getHitCount(): number;
  resetHitCount(): void;
...
export function createWhoopBodyMeasurementsHelper(): WhoopBodyMeasurementsHelper {
  let hitCount = 0;
  let nextResponse: NextResponse | null = null;
  let lastRequestUrl: URL | null = null;

  const handler: HttpHandler = http.get(BODY_MEASUREMENTS_URL, ({ request }) => {
    hitCount += 1;
