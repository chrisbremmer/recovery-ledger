// MSW helper for the WHOOP `/v2/activity/workout` endpoint. Sibling to
// `msw-whoop-cycles.ts`; same shape, different endpoint + fixtures dir.
// See `msw-whoop-cycles.ts` for the full rationale.
...
export const WORKOUTS_URL = 'https://api.prod.whoop.com/v2/activity/workout';
...
export interface WhoopWorkoutsHelper {
  server: SetupServer;
  getHitCount(): number;
  resetHitCount(): void;
...
export function createWhoopWorkoutsHelper(): WhoopWorkoutsHelper {
  let hitCount = 0;
  let nextResponse: NextResponse | null = null;
  let lastRequestUrl: URL | null = null;

  const handler: HttpHandler = http.get(WORKOUTS_URL, ({ request }) => {
    hitCount += 1;
