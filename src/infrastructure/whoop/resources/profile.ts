...
//
// Phase 10 ARCH-03: factory shape — `createGetProfile({authedCall})`
...
export function createGetProfile(deps: GetProfileDeps) {
  return async function getProfile(): Promise<GetProfileResult> {
    const raw = await httpGet('/v2/user/profile/basic', {}, WhoopRawProfile, deps.authedCall);
